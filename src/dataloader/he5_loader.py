...
class HE5ThermalLoader:
    ...
    def _process_thermal_data(self, thermal_data: np.ndarray) -> torch.Tensor:
        ...
        # NO CLIPPING - Preserve original thermal data range for better anomaly detection
        # Original clipping removed to prevent data loss
        # data = np.clip(data, self.temperature_range[0], self.temperature_range[1])
        
        # Normalize if requested (using actual data range, not clipped range)
        if self.normalize:
            data = self._normalize_temperature_full_range(data)
    ...
    def _normalize_temperature_full_range(self, data: np.ndarray) -> np.ndarray:
        """Normalize temperature data to [0, 1] range using actual data min/max (NO DATA LOSS)."""
        data_min = np.min(data)
        data_max = np.max(data)
        
        if data_max == data_min:
            # Uniform data, return zeros
            return np.zeros_like(data)
        
        # Normalize using actual data range to preserve all thermal information
